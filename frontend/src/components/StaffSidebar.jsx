...
export default function StaffSidebar() {
  ...
  return (
    <aside className={clsx(
      "w-60 h-screen border-r transition-colors",
      isDark
      ...
      <div className={clsx(
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className={clsx("text-base font-bold", isDark ? "text-white" : "text-slate-900")}>Staff Panel</h1>
            <p className={clsx("text-xs", isDark ? "text-slate-400" : "text-slate-500")}>QueueLess Print</p>
          </div>
        </div>
        <div className={clsx("mt-3 inline-flex items-center gap-1.5 text-xs font-medium px-2 py-1 rounded-full", isDark ? "bg-emerald-500/20 text-emerald-300" : "bg-emerald-100 text-emerald-700")}>
          <Sparkles size={12} />
      ...
      <div className={clsx(
        ...
        <button
          onClick={logout}
          className={clsx(
            "w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl font-medium transition-all",
            isDark
              ? "text-slate-300 hover:bg-slate-800 hover:text-white"
              : "text-slate-700 hover:bg-slate-100"
          )}
